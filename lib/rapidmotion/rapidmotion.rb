module RapidMotion
	...
	def add_button(args)
		...
		bottom=args[:bottom]
		if !bottom.nil?
			b1.frame = [[40, content_frame.height-40-(60*(1+bottom))], [view.frame.size.width - 40 * 2, 40]]
		else
			top = args[:top] || 0
  ...
  def textFieldShouldReturn(textField)
	...
	def background_image(i)
		self.view.backgroundColor = "#{i}".uiimage.scale_within([view.frame.size.width,view.frame.size.height]).uicolor
	end
